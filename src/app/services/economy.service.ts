...
export class EconomyService {
  ...
  computeAiReserveDump(gridSize: number, aiBase: Position, reserves: number, getUnitAt: (x: number, y: number) => Unit | null): { created: Unit[], remaining: number } {
    ...
    while (aiRes > 0 && adj.length > 0) {
      ...
      created.push({ id: crypto.randomUUID(), position: { ...target }, level: tl.level, tier: tl.tier, points: cost, owner: 'ai', turnsStationary: 0, forestOccupationTurns: 0, productionActive: false, hasActed: true });
      aiRes -= cost;
    }
